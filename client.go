...
func NewClientWithConfig(trackerAddr []string, maxConn int) (*Client, error) {
	client := &Client{
		maxConns:          maxConn,
		storagePoolLock: &sync.RWMutex{},
	}
	...
	for _, addr := range trackerAddr {
		trackerPool, err := newConnPool(addr, maxConn)
		if err != nil {
			return nil, err
		}
...
func (this *Client) getStorageConn(storageInfo *storageInfo) (net.Conn, error) {
	...
	storagePool, err := newConnPool(storageInfo.addr, this.maxConns)
	if err != nil {
		this.storagePoolLock.Unlock()
		return nil, err
	}
